...
class MessengerController extends Controller
{
    public function Index(Request $request){
        ...
        if($request->session()->has('user_id') && $request->session()->has('acmStatus')){
			...
			$msgData = DB::table('message_details')
						->leftJoin('user_details','user_details.user_id','=','message_details.sender_id')
			...
			return view('AccountManagerViews.messenger', ['user' => $user,'data' => $data,'msgData' => $msgData,'postCount' => $postCount,'acccCount' => $acccCount,'accsCount' => $accsCount,'blckCount' => $blckCount]);
        }
        else{
	...
	public function Messenger(Request $request){
        if($request->session()->has('user_id')){
			...
			console.log(".............................................");
	        $user_id = $request->session()->get('user_id');
	        // $msgS=DB::table('message_details')
							// ->where([['sender_id', '=' , $user_id],['receiver_id', '=' , $id]])
							// ->orWhere([['receiver_id', '=' , $user_id],['sender_id', '=' , $id]])
							// ->orderBy('message_time', 'desc')
							// ->get();

            // foreach($msgS as $ms){
				// $msData .= $ms->sender_id.' '.$ms->message_text.' '.$ms->message_time.'<br>';
			// }

	        return response()->json(array('msData'=> $id), 200);
        }
        else{
            return redirect('/');
        }
